...
#ifndef DEMO_RESOURCE_PATH
#define DEMO_RESOURCE_PATH "../example"
#endif
...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	for (i = 0; i < 12; i++) {
		char file[512];
		snprintf(file, sizeof(file), "%s/images/image%d.jpg", DEMO_RESOURCE_PATH, i+1);
	...
	{
		char file[512];
		snprintf(file, sizeof(file), "%s/entypo.ttf", DEMO_RESOURCE_PATH);
		data->fontIcons = nvgCreateFont(vg, "icons", file);
		if (data->fontIcons == -1) {
			printf("Could not add font icons.\n");
			return -1;
		}
		snprintf(file, sizeof(file), "%s/Roboto-Regular.ttf", DEMO_RESOURCE_PATH);
		data->fontNormal = nvgCreateFont(vg, "sans", file);
		if (data->fontNormal == -1) {
			printf("Could not add font italic.\n");
			return -1;
		}
		snprintf(file, sizeof(file), "%s/Roboto-Bold.ttf", DEMO_RESOURCE_PATH);
		data->fontBold = nvgCreateFont(vg, "sans-bold", file);
		if (data->fontBold == -1) {
			printf("Could not add font bold.\n");
			return -1;
		}
		snprintf(file, sizeof(file), "%s/NotoEmoji-Regular.ttf", DEMO_RESOURCE_PATH);
	...
	nvgSetEmojiFontId(vg, data->fontEmoji);
...
void renderDemoStatic(NVGcontext* vg, float width, float height, DemoData* data)
{
	float x, y, popy;
	NVG_NOTUSED(width);
	NVG_NOTUSED(height);

	// Line styles (no animation)
	drawWidths(vg, 10, 50, 30);
	drawCaps(vg, 10, 300, 30);

	// Widgets chrome — fixed content for display-list capture
	...
	// Freeze thumbnail scroll/spinner at t=0 so geometry can be retained.
	drawThumbnails(vg, 365, popy - 30, 160, 300, data->images, 12, 0.0f);
}

void renderDemoDynamic(NVGcontext* vg, float mx, float my, float width, float height, float t, DemoData* data)
{
	NVG_NOTUSED(data);
				...
				float t, int blowup, DemoData* data)
{
	...
	renderDemoDynamic(vg, mx, my, width, height, t, data);

	// Immediate-mode static style samples + live widgets (including animated thumbs).
	drawWidths(vg, 10, 50, 30);
	drawCaps(vg, 10, 300, 30);
