...
// Path 2 tessellated draw-list IR (also used by Path 1 display-list recording).
enum NVGdrawCallType {
	NVG_DRAW_NONE = 0,
	NVG_DRAW_FILL,
	NVG_DRAW_STROKE,
	NVG_DRAW_TRIANGLES,
...
typedef struct NVGdrawPath NVGdrawPath;
...
typedef struct NVGdrawCall NVGdrawCall;
...
struct NVGdrawList {
	...
	int nverts;
	int cverts;
	// Scratch path array reused when submitting fills/strokes to backends.
	NVGpath* pathScratch;
	int cpathScratch;
	float devicePxRatio;
...
struct NVGcontext {
	...
	int textTriCount;
	int emojiFontId;
	// Active Path 1 recording target (NULL = capture into the per-frame list).
	NVGdrawList* recording;
	// Per-frame Path 2 IR for immediate-mode drawing.
	NVGdrawList frameList;
	// Scratch vertices used when submitting a retained list under a non-identity transform.
	NVGvertex* submitVerts;
	int csubmitVerts;
};
...
// Forward declarations used by display-list recording / end.
...
static void nvg__drawListFreeBuffers(NVGdrawList* list);
static void nvg__drawListClear(NVGdrawList* list);
static void nvg__drawListSubmit(NVGcontext* ctx, const NVGdrawList* list, int useStateXform);
static void nvg__flushFrameList(NVGcontext* ctx);
...
void nvgDeleteInternal(NVGcontext* ctx)
{
	...
	nvg__drawListFreeBuffers(&ctx->frameList);
	free(ctx->submitVerts);
...
{
	// Path 2: submit any remaining immediate-mode packets, then flush the backend.
...
//
// Draw lists (Path 2 IR) + display-list recording façade (Path 1)
//

static int nvg__dlMaxi(int a, int b) { return a > b ? a : b; }
...
static void nvg__drawListFreeBuffers(NVGdrawList* list)
{
	...
	free(list->paths);
	free(list->verts);
	free(list->pathScratch);
	list->calls = NULL;
	list->paths = NULL;
	list->verts = NULL;
	list->pathScratch = NULL;
	list->ncalls = list->ccalls = 0;
	list->npaths = list->cpaths = 0;
	list->nverts = list->cverts = 0;
	list->cpathScratch = 0;
}

void nvgClearDrawList(NVGdrawList* list)
{
	nvg__drawListClear(list);
}

NVGdrawList* nvgCreateDrawList(NVGcontext* ctx)
{
	NVGdrawList* list;
	NVG_NOTUSED(ctx);
	list = (NVGdrawList*)malloc(sizeof(NVGdrawList));
	if (list == NULL) return NULL;
	memset(list, 0, sizeof(NVGdrawList));
	return list;
}

void nvgDeleteDrawList(NVGcontext* ctx, NVGdrawList* list)
{
	if (list == NULL) return;
	if (ctx != NULL && ctx->recording == list)
		ctx->recording = NULL;
	nvg__drawListFreeBuffers(list);
	free(list);
}

int nvgDrawListSize(const NVGdrawList* list)
{
	if (list == NULL) return 0;
	return list->ncalls;
}

int nvgDrawListNeedsRebuild(NVGcontext* ctx, const NVGdrawList* list)
{
...
// Path 1 aliases
void nvgClearDisplayList(NVGdisplayList* dl) { nvgClearDrawList(dl); }
NVGdisplayList* nvgCreateDisplayList(NVGcontext* ctx) { return nvgCreateDrawList(ctx); }
void nvgDeleteDisplayList(NVGcontext* ctx, NVGdisplayList* dl) { nvgDeleteDrawList(ctx, dl); }
int nvgDisplayListSize(const NVGdisplayList* dl) { return nvgDrawListSize(dl); }
int nvgDisplayListNeedsRebuild(NVGcontext* ctx, const NVGdisplayList* dl) { return nvgDrawListNeedsRebuild(ctx, dl); }

int nvgIsRecordingDisplayList(NVGcontext* ctx)
{
	if (ctx == NULL) return 0;
	return ctx->recording != NULL;
}

static NVGdrawList* nvg__captureTarget(NVGcontext* ctx)
{
	if (ctx == NULL) return NULL;
	if (ctx->recording != NULL) return ctx->recording;
	return &ctx->frameList;
}

static NVGdrawCall* nvg__dlAllocCall(NVGdrawList* list)
{
	NVGdrawCall* ret;
	if (list->ncalls + 1 > list->ccalls) {
		NVGdrawCall* calls;
		int ccalls = nvg__dlMaxi(list->ncalls + 1, NVG_INIT_DL_CALLS_SIZE) + list->ccalls / 2;
		calls = (NVGdrawCall*)realloc(list->calls, sizeof(NVGdrawCall) * ccalls);
		if (calls == NULL) return NULL;
		list->calls = calls;
		list->ccalls = ccalls;
	}
	ret = &list->calls[list->ncalls++];
	memset(ret, 0, sizeof(NVGdrawCall));
	return ret;
}

static int nvg__dlAllocPaths(NVGdrawList* list, int n)
{
	int ret = list->npaths;
	if (n <= 0) return ret;
	if (list->npaths + n > list->cpaths) {
		NVGdrawPath* paths;
		int cpaths = nvg__dlMaxi(list->npaths + n, NVG_INIT_DL_PATHS_SIZE) + list->cpaths / 2;
		paths = (NVGdrawPath*)realloc(list->paths, sizeof(NVGdrawPath) * cpaths);
...
static int nvg__dlAllocVerts(NVGdrawList* list, int n)
{
	int ret = list->nverts;
	if (n <= 0) return ret;
	if (list->nverts + n > list->cverts) {
		NVGvertex* verts;
		int cverts = nvg__dlMaxi(list->nverts + n, NVG_INIT_DL_VERTS_SIZE) + list->cverts / 2;
		verts = (NVGvertex*)realloc(list->verts, sizeof(NVGvertex) * cverts);
...
static int nvg__dlCapturePaths(NVGdrawList* list, const NVGpath* paths, int npaths, int* pathOffsetOut)
{
	int pathOffset, i;
	if (npaths <= 0) {
		*pathOffsetOut = 0;
	...
	pathOffset = nvg__dlAllocPaths(list, npaths);
	if (pathOffset < 0) return 0;
	for (i = 0; i < npaths; i++) {
		NVGdrawPath* dst = &list->paths[pathOffset + i];
		const NVGpath* src = &paths[i];
		memset(dst, 0, sizeof(NVGdrawPath));
		dst->winding = src->winding;
		dst->convex = src->convex;
		dst->closed = src->closed;
		if (src->nfill > 0 && src->fill != NULL) {
			int off = nvg__dlAllocVerts(list, src->nfill);
			if (off < 0) return 0;
			memcpy(&list->verts[off], src->fill, sizeof(NVGvertex) * src->nfill);
			dst->fillOffset = off;
			dst->fillCount = src->nfill;
		}
		if (src->nstroke > 0 && src->stroke != NULL) {
			int off = nvg__dlAllocVerts(list, src->nstroke);
			if (off < 0) return 0;
			memcpy(&list->verts[off], src->stroke, sizeof(NVGvertex) * src->nstroke);
			dst->strokeOffset = off;
			dst->strokeCount = src->nstroke;
		}
	}
	*pathOffsetOut = pathOffset;
	return 1;
}

static void nvg__dlCaptureFill(NVGdrawList* list, NVGpaint* paint, NVGcompositeOperationState compositeOperation,
	NVGscissor* scissor, float fringe, const float* bounds, const NVGpath* paths, int npaths)
{
	NVGdrawCall* call;
	int pathOffset = 0;
	if (list == NULL) return;
	call = nvg__dlAllocCall(list);
	if (call == NULL) return;
	if (!nvg__dlCapturePaths(list, paths, npaths, &pathOffset)) {
		list->ncalls--;
		return;
	}
	call->type = NVG_DRAW_FILL;
	call->paint = *paint;
	...
	memcpy(call->bounds, bounds, sizeof(float) * 4);
}

static void nvg__dlCaptureStroke(NVGdrawList* list, NVGpaint* paint, NVGcompositeOperationState compositeOperation,
...
static void nvg__dlCaptureTriangles(NVGdrawList* list, NVGpaint* paint, NVGcompositeOperationState compositeOperation,
	NVGscissor* scissor, const NVGvertex* verts, int nverts, float fringe)
{
	NVGdrawCall* call;
	int triOffset;
	if (list == NULL || nverts <= 0 || verts == NULL) return;
	call = nvg__dlAllocCall(list);
	if (call == NULL) return;
	triOffset = nvg__dlAllocVerts(list, nverts);
	if (triOffset < 0) {
		list->ncalls--;
		return;
	}
	memcpy(&list->verts[triOffset], verts, sizeof(NVGvertex) * nverts);
	call->type = NVG_DRAW_TRIANGLES;
	call->paint = *paint;
	call->compositeOperation = compositeOperation;
	call->scissor = *scissor;
	call->fringe = fringe;
	call->triangleOffset = triOffset;
	call->triangleCount = nverts;
}

static int nvg__dlEnsurePathScratch(NVGdrawList* list, int npaths)
{
	if (npaths <= list->cpathScratch) return 1;
	{
		NVGpath* paths = (NVGpath*)realloc(list->pathScratch, sizeof(NVGpath) * npaths);
		if (paths == NULL) return 0;
		list->pathScratch = paths;
		list->cpathScratch = npaths;
	}
	return 1;
}

static int nvg__xformIsIdentity(const float* t)
{
	return t[0] == 1.0f && t[1] == 0.0f && t[2] == 0.0f &&
		t[3] == 1.0f && t[4] == 0.0f && t[5] == 0.0f;
}

// Deep-copy src into dst. When xform is non-NULL and non-identity, geometry,
// paint transforms, scissors and fill bounds are transformed into the parent
// list's coordinate space. This is required when a parent is recording and a
// child submits a retained local-space list under nvgTranslate(child.pos).
static void nvg__dlAppendList(NVGdrawList* dst, const NVGdrawList* src, const float* xform)
{
	int i;
	int applyXform = (xform != NULL && !nvg__xformIsIdentity(xform));
	if (dst == NULL || src == NULL || src->ncalls == 0) return;
	for (i = 0; i < src->ncalls; i++) {
		const NVGdrawCall* sc = &src->calls[i];
		NVGdrawCall* dc = nvg__dlAllocCall(dst);
		int j;
		if (dc == NULL) return;
		*dc = *sc;
		if (applyXform) {
			nvgTransformPremultiply(dc->paint.xform, xform);
			if (dc->scissor.extent[0] >= 0.0f)
				nvgTransformPremultiply(dc->scissor.xform, xform);
			if (sc->type == NVG_DRAW_FILL) {
				float x0, y0, x1, y1, x2, y2, x3, y3;
				nvgTransformPoint(&x0, &y0, xform, sc->bounds[0], sc->bounds[1]);
				nvgTransformPoint(&x1, &y1, xform, sc->bounds[2], sc->bounds[1]);
				nvgTransformPoint(&x2, &y2, xform, sc->bounds[2], sc->bounds[3]);
				nvgTransformPoint(&x3, &y3, xform, sc->bounds[0], sc->bounds[3]);
				dc->bounds[0] = nvg__minf(nvg__minf(x0, x1), nvg__minf(x2, x3));
				dc->bounds[1] = nvg__minf(nvg__minf(y0, y1), nvg__minf(y2, y3));
				dc->bounds[2] = nvg__maxf(nvg__maxf(x0, x1), nvg__maxf(x2, x3));
				dc->bounds[3] = nvg__maxf(nvg__maxf(y0, y1), nvg__maxf(y2, y3));
			}
		}
		if (sc->type == NVG_DRAW_FILL || sc->type == NVG_DRAW_STROKE) {
			int pathOffset = nvg__dlAllocPaths(dst, sc->pathCount);
			if (pathOffset < 0) { dst->ncalls--; return; }
			dc->pathOffset = pathOffset;
			for (j = 0; j < sc->pathCount; j++) {
				const NVGdrawPath* sp = &src->paths[sc->pathOffset + j];
				NVGdrawPath* dp = &dst->paths[pathOffset + j];
				*dp = *sp;
				if (sp->fillCount > 0) {
					int off = nvg__dlAllocVerts(dst, sp->fillCount);
					int k;
					if (off < 0) { dst->ncalls--; return; }
					if (applyXform) {
						for (k = 0; k < sp->fillCount; k++) {
							const NVGvertex* sv = &src->verts[sp->fillOffset + k];
							NVGvertex* dv = &dst->verts[off + k];
							nvgTransformPoint(&dv->x, &dv->y, xform, sv->x, sv->y);
							dv->u = sv->u;
							dv->v = sv->v;
						}
					} else {
						memcpy(&dst->verts[off], &src->verts[sp->fillOffset], sizeof(NVGvertex) * sp->fillCount);
					}
					dp->fillOffset = off;
				}
				if (sp->strokeCount > 0) {
					int off = nvg__dlAllocVerts(dst, sp->strokeCount);
					int k;
					if (off < 0) { dst->ncalls--; return; }
					if (applyXform) {
						for (k = 0; k < sp->strokeCount; k++) {
							const NVGvertex* sv = &src->verts[sp->strokeOffset + k];
							NVGvertex* dv = &dst->verts[off + k];
							nvgTransformPoint(&dv->x, &dv->y, xform, sv->x, sv->y);
							dv->u = sv->u;
							dv->v = sv->v;
						...
						memcpy(&dst->verts[off], &src->verts[sp->strokeOffset], sizeof(NVGvertex) * sp->strokeCount);
					}
					dp->strokeOffset = off;
				}
			}
		} else if (sc->type == NVG_DRAW_TRIANGLES) {
			int off = nvg__dlAllocVerts(dst, sc->triangleCount);
			if (off < 0) { dst->ncalls--; return; }
			if (applyXform) {
				int k;
				for (k = 0; k < sc->triangleCount; k++) {
					const NVGvertex* sv = &src->verts[sc->triangleOffset + k];
					NVGvertex* dv = &dst->verts[off + k];
					nvgTransformPoint(&dv->x, &dv->y, xform, sv->x, sv->y);
					dv->u = sv->u;
					dv->v = sv->v;
				...
				memcpy(&dst->verts[off], &src->verts[sc->triangleOffset], sizeof(NVGvertex) * sc->triangleCount);
			}
			dc->triangleOffset = off;
		}
	}
}

void nvgCloneDrawList(NVGdrawList* dst, const NVGdrawList* src)
{
	if (dst == NULL) return;
...
static int nvg__ensureSubmitVerts(NVGcontext* ctx, int n)
{
	if (n <= ctx->csubmitVerts) return 1;
	{
		NVGvertex* verts;
		int cverts = nvg__dlMaxi(n, 256) + ctx->csubmitVerts / 2;
		verts = (NVGvertex*)realloc(ctx->submitVerts, sizeof(NVGvertex) * cverts);
		if (verts == NULL) return 0;
		ctx->submitVerts = verts;
		ctx->csubmitVerts = cverts;
	}
	return 1;
}

static void nvg__transformVerts(NVGvertex* dst, const NVGvertex* src, int n, const float* xform)
{
	int i;
	for (i = 0; i < n; i++) {
		nvgTransformPoint(&dst[i].x, &dst[i].y, xform, src[i].x, src[i].y);
		dst[i].u = src[i].u;
		dst[i].v = src[i].v;
	}
}
...
{
	int i, j;
	NVGdrawList* mutableList = (NVGdrawList*)list;
	NVGstate* state;
	int applyXform = 0;
	float alpha = 1.0f;
	...
	if (useStateXform) {
		applyXform = !nvg__xformIsIdentity(state->xform);
		alpha = state->alpha;
	}
	for (i = 0; i < list->ncalls; i++) {
		const NVGdrawCall* call = &list->calls[i];
		NVGpaint paint = call->paint;
		NVGscissor scissor = call->scissor;
		float bounds[4];
		...
		if (useStateXform && alpha != 1.0f) {
			paint.innerColor.a *= alpha;
			paint.outerColor.a *= alpha;
		}

		if (call->type == NVG_DRAW_FILL || call->type == NVG_DRAW_STROKE) {
			int vertCount = 0;
			int vertBase = 0;
			if (!nvg__dlEnsurePathScratch(mutableList, call->pathCount))
				continue;

			// Count verts for optional xform buffer.
			if (applyXform) {
				for (j = 0; j < call->pathCount; j++) {
					const NVGdrawPath* sp = &list->paths[call->pathOffset + j];
					vertCount += sp->fillCount + sp->strokeCount;
				}
				if (!nvg__ensureSubmitVerts(ctx, vertCount))
					continue;
			}

			for (j = 0; j < call->pathCount; j++) {
				const NVGdrawPath* sp = &list->paths[call->pathOffset + j];
				NVGpath* dp = &mutableList->pathScratch[j];
				memset(dp, 0, sizeof(NVGpath));
				dp->winding = sp->winding;
				dp->convex = sp->convex;
				dp->closed = sp->closed;
				if (sp->fillCount > 0) {
					if (applyXform) {
						nvg__transformVerts(&ctx->submitVerts[vertBase], &list->verts[sp->fillOffset],
							sp->fillCount, state->xform);
						dp->fill = &ctx->submitVerts[vertBase];
						vertBase += sp->fillCount;
					} else {
						dp->fill = &mutableList->verts[sp->fillOffset];
					}
					dp->nfill = sp->fillCount;
				}
				if (sp->strokeCount > 0) {
					if (applyXform) {
						nvg__transformVerts(&ctx->submitVerts[vertBase], &list->verts[sp->strokeOffset],
							sp->strokeCount, state->xform);
						dp->stroke = &ctx->submitVerts[vertBase];
						vertBase += sp->strokeCount;
					} else {
						dp->stroke = &mutableList->verts[sp->strokeOffset];
					}
					dp->nstroke = sp->strokeCount;
				}
			}

			if (call->type == NVG_DRAW_FILL) {
				const float* boundsPtr = call->bounds;
				if (applyXform) {
					// Transform AABB corners; approximate bounds for stencil fills.
					float x0, y0, x1, y1, x2, y2, x3, y3;
					nvgTransformPoint(&x0, &y0, state->xform, call->bounds[0], call->bounds[1]);
					nvgTransformPoint(&x1, &y1, state->xform, call->bounds[2], call->bounds[1]);
					nvgTransformPoint(&x2, &y2, state->xform, call->bounds[2], call->bounds[3]);
					nvgTransformPoint(&x3, &y3, state->xform, call->bounds[0], call->bounds[3]);
					bounds[0] = nvg__minf(nvg__minf(x0, x1), nvg__minf(x2, x3));
					bounds[1] = nvg__minf(nvg__minf(y0, y1), nvg__minf(y2, y3));
					bounds[2] = nvg__maxf(nvg__maxf(x0, x1), nvg__maxf(x2, x3));
					bounds[3] = nvg__maxf(nvg__maxf(y0, y1), nvg__maxf(y2, y3));
					boundsPtr = bounds;
				}
				ctx->params.renderFill(ctx->params.userPtr, &paint, call->compositeOperation,
					(NVGscissor*)scissorPtr, call->fringe, boundsPtr, mutableList->pathScratch, call->pathCount);
			} else {
				ctx->params.renderStroke(ctx->params.userPtr, &paint, call->compositeOperation,
					(NVGscissor*)scissorPtr, call->fringe, call->strokeWidth, mutableList->pathScratch, call->pathCount);
			}
		} else if (call->type == NVG_DRAW_TRIANGLES) {
			const NVGvertex* triVerts = &list->verts[call->triangleOffset];
			if (applyXform) {
				if (!nvg__ensureSubmitVerts(ctx, call->triangleCount))
					continue;
				nvg__transformVerts(ctx->submitVerts, triVerts, call->triangleCount, state->xform);
				triVerts = ctx->submitVerts;
			}
			ctx->params.renderTriangles(ctx->params.userPtr, &paint, call->compositeOperation,
				(NVGscissor*)scissorPtr, triVerts, call->triangleCount, call->fringe);
		}
	}
}
...
{
	if (ctx == NULL) return;
	if (ctx->frameList.ncalls <= 0) return;
	// Frame list is already in destination/world space.
	nvg__drawListSubmit(ctx, &ctx->frameList, 0);
	nvg__drawListClear(&ctx->frameList);
}

void nvgSubmitDrawList(NVGcontext* ctx, const NVGdrawList* list)
{
	NVGstate* state;
	if (ctx == NULL || list == NULL) return;
	state = nvg__getState(ctx);
	// Nested into an active recording: splice packets into the parent list in the
	// *current* coordinate space (child local-space lists are transformed by
	// nvgTranslate(child.pos) etc.). Without this, nested caches draw at the
	// parent's origin (e.g. over a Window title bar).
	if (ctx->recording != NULL) {
		if (ctx->recording != list)
			nvg__dlAppendList(ctx->recording, list, state->xform);
		return;
	}
	// Preserve draw order vs immediate mode: flush any pending frame packets first,
	// then submit the retained list, applying the current transform for local-space lists.
	nvg__flushFrameList(ctx);
	nvg__drawListSubmit(ctx, list, 1);
}

void nvgBeginDisplayList(NVGcontext* ctx, NVGdisplayList* dl)
{
	if (ctx == NULL || dl == NULL) return;
	// Flush any in-progress immediate packets so recording starts from a clean frame IR.
	nvg__flushFrameList(ctx);
	nvgClearDrawList(dl);
...
void nvgEndDisplayList(NVGcontext* ctx)
{
	if (ctx == NULL) return;
	// Ensure font atlas GPU texture matches UVs captured during text recording.
	nvg__flushTextTexture(ctx);
	ctx->recording = NULL;
}

void nvgDrawDisplayList(NVGcontext* ctx, NVGdisplayList* dl)
{
	nvgSubmitDrawList(ctx, dl);
}
...
void nvgFill(NVGcontext* ctx)
{
	...
	// Retained display lists (recording): capture tessellated IR.
	// Immediate mode: submit directly to the backend (original NanoVG path).
	if (ctx->recording != NULL) {
		nvg__dlCaptureFill(ctx->recording, &fillPaint, state->compositeOperation, &state->scissor,
			ctx->fringeWidth, ctx->cache->bounds, ctx->cache->paths, ctx->cache->npaths);
	} else {
...
void nvgStroke(NVGcontext* ctx)
{
	...
	if (ctx->recording != NULL) {
		nvg__dlCaptureStroke(ctx->recording, &strokePaint, state->compositeOperation, &state->scissor,
...
static void nvg__renderText(NVGcontext* ctx, NVGvertex* verts, int nverts, NVGpaint paint)
{
    ...
    if (ctx->recording != NULL) {
        nvg__dlCaptureTriangles(ctx->recording, &paint, state->compositeOperation, &state->scissor,
