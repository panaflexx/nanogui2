/*
 ...
 Options:
   ...
   -s, --scale <factor>   Scale all widget sizes/positions/font sizes by
                          <factor> before emitting C++ (default: 1.0).
                          This mirrors the runtime --scale option of
                          ngserver, but the scaling is baked into the
                          generated source.
...
struct Gen {
    ...
    // Scale an integer value (position / size / font_size, etc.).
    int s(int v) const { return (int)std::round((double)v * (double)scale); }
};
...
static void gen_widget(Gen& g, DictValue* obj, const std::string& parent_var) {
    ...
    if (get_vec2i(obj, "position", x, y))
        g.body << g.indent() << var << "->set_position(nanogui::Vector2i("
               << g.s(x) << ", " << g.s(y) << "));\n";
    if (get_vec2i(obj, "size", x, y))
        g.body << g.indent() << var << "->set_size(nanogui::Vector2i("
               << g.s(x) << ", " << g.s(y) << "));\n";
    if (get_vec2i(obj, "fixed_size", x, y))
        g.body << g.indent() << var << "->set_fixed_size(nanogui::Vector2i("
               << g.s(x) << ", " << g.s(y) << "));\n";

    // Explicit font_size from JSON -- bake the scaled value.
    {
        DictValue* fv = get_field(obj, "font_size");
        double fnum = 0.0;
        if (get_number_val(fv, fnum)) {
            int fs = (int)std::round(fnum * (double)g.scale);
            g.body << g.indent() << var << "->set_font_size(" << fs << ");\n";
        }
    }
    ...
    // from the theme default).  This mirrors ngserver's applyScaleRecursive.
    // We only emit this when no explicit font_size was specified in JSON,
    // because in that case we already baked in the scaled literal above.
    if (g.scale != 1.0f && !get_field(obj, "font_size")) {
        std::ostringstream sf;
        sf.precision(9);
        sf << g.scale;
        std::string sfs = sf.str();
        if (sfs.find('.') == std::string::npos && sfs.find('e') == std::string::npos
            && sfs.find('E') == std::string::npos)
            sfs += ".0";
        g.body << g.indent() << var << "->set_font_size((int)std::round("
               << var << "->font_size() * " << sfs << "f));\n";
    }
...
static std::string make_source(const std::string& classname,
    ...
    if (scale != 1.0f) {
        s << "    // Scale factor applied at code-generation time: "
          << scale << "\n";
    }
...
static std::string make_mainapp(const std::string& classname,
    ...
    if (scale > 0.0f && scale != 1.0f) {
        win_w = (int)std::round((double)win_w * (double)scale);
        win_h = (int)std::round((double)win_h * (double)scale);
    }
...
static void print_usage(const char* prog) {
    std::cerr <<
        ...
        "  -s, --scale <factor> Scale widget sizes / positions / font sizes by\n"
        "                       <factor> before writing the C++ (default: 1.0)\n"
...
int main(int argc, char** argv) {
    ...
    std::string cpp_src    = make_source(classname, root, canvas_w, canvas_h, scale);
    ...
    if (fullmain) {
        std::string main_src = make_mainapp(classname, canvas_w, canvas_h, scale);
